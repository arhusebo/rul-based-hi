...
fig_path = "../validation/spectrum_prediction"
...
for dataset in metadata_all["test_data"]:
    ...
    test_gen = datagen.SpectrumDataSequence(metadata, "test", models.basic_hi,
                                        batch_size=1,
...
stat_data.to_json(f"{stat_data_path}/spectrum_stats.json")
print(stat_data)

data = pd.DataFrame({"trues":trues, "preds":preds})
data.to_json("validation/spectrum_data.json")

plotr.plot_hi_subplots(trues, preds, names=legend)
plt.savefig(f"{fig_path}/all.png")
plt.show()